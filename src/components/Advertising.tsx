...
const stats = [
  { value: '30 500+', label: 'Активных подписчиков', icon: <Users className="text-blue-400" /> },
...
export const Advertising: React.FC = () => {
  return (
    <section id="advertising" className="py-24 bg-neutral-950 relative overflow-hidden">
      ...
      {/* Decorative Marquee */}
      <div className="absolute top-10 left-0 w-full overflow-hidden opacity-5 pointer-events-none rotate-3">
        <motion.div
          animate={{ x: [0, -1000] }}
          transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
          className="whitespace-nowrap font-display text-9xl font-bold text-white flex gap-12"
        >
          <span>PREMIUM SPONSORS</span>
          <span>•</span>
          <span>HIGH CONVERSION</span>
          <span>•</span>
          <span>VIP AUDIENCE</span>
          <span>•</span>
          <span>TOP ROI</span>
          <span>•</span>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-20">
          {stats.map((stat, i) => (
            <motion.div 
              ...
              className={`p-6 rounded-3xl bg-neutral-900 border border-white/5 text-center shadow-lg ${i % 2 === 1 ? 'animate-float' : 'animate-float-delayed'}`}
            >
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {formats.map((format, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ delay: i * 0.1 }}
              className={`h-full ${i % 2 === 0 ? 'animate-float-delayed' : 'animate-float'}`}
            >
              <div className="group rounded-[2rem] bg-gradient-to-b from-neutral-900 to-neutral-950 border border-white/10 hover:border-blue-500/50 hover:shadow-[0_0_30px_rgba(37,99,235,0.2)] transition-all flex flex-col h-full overflow-hidden relative">
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-[100%] group-hover:animate-[shimmer_2s_infinite] pointer-events-none z-20" />
                ...
                <div className="p-8 flex flex-col flex-1">
                  <h4 className="text-2xl font-bold mb-4 z-10">{format.title}</h4>
                  <p className="text-neutral-400 text-sm mb-6 flex-1 z-10">{format.desc}</p>
                  
                  <div className="space-y-4 pt-6 border-t border-white/5 mt-auto z-10">
                    ...
                    <a href="#managers" className="w-full block text-center py-3 mt-4 rounded-xl bg-white/5 hover:bg-blue-600 hover:text-white transition-all text-sm font-bold text-neutral-300 relative overflow-hidden group/btn">
                      <div className="absolute inset-0 bg-gradient-to-r from-blue-600 to-emerald-600 opacity-0 group-hover/btn:opacity-100 transition-opacity" />
