        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {advantages.map((item, index) => (
            <motion.div
              ...
              className="h-full"
            >
              <div className={`group p-8 rounded-3xl bg-neutral-800/60 backdrop-blur-md border border-white/20 hover:border-emerald-400/60 transition-all shadow-xl hover:shadow-[0_0_60px_rgba(16,185,129,0.4)] relative overflow-hidden h-full flex flex-col ${index % 2 === 0 ? 'animate-float' : 'animate-float-delayed'}`}>
                <div className="absolute inset-0 bg-gradient-to-br from-blue-500/20 via-transparent to-emerald-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                <div className="absolute top-0 right-0 w-40 h-40 bg-emerald-400/30 blur-[60px] opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                <div className="absolute bottom-0 left-0 w-40 h-40 bg-blue-400/30 blur-[60px] opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                
                <div className="h-16 w-16 rounded-2xl bg-white/10 flex items-center justify-center mb-6 border border-white/30 group-hover:border-emerald-400/60 group-hover:bg-emerald-400/30 transition-all group-hover:scale-110 group-hover:rotate-6 shadow-[0_0_30px_rgba(0,0,0,0.5)] group-hover:shadow-[0_0_40px_rgba(16,185,129,0.8)] relative z-10 shrink-0">
                  {item.icon}
                </div>
                <h3 className="text-xl font-bold mb-3 relative z-10 text-white group-hover:text-emerald-300 transition-colors drop-shadow-md">{item.title}</h3>
                <p className="text-neutral-300 font-medium leading-relaxed relative z-10 transition-colors group-hover:text-white flex-1">
                  {item.description}
                </p>
                
                <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-blue-500/30 blur-[60px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
              </div>
            </motion.div>
          ))}
        </div>
